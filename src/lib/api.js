...
class ApiClient {
    ...
    // ===== ROOMS =====
    async createRoom() {
        return this.request('/rooms', { method: 'POST' });
    }
    ...
    async spectateRoom(code) {
        return this.request(`/rooms/${code}/spectate`, { method: 'POST' });
    }

    async leaveRoom(code) {
        return this.request(`/rooms/${code}/leave`, { method: 'POST' });
    }

    // ===== GAME =====
    async makeMove(roomId, position, version) {
        return this.request(`/game/${roomId}/move`, {
            method: 'POST',
    ...
    async getGameState(roomId) {
        return this.request(`/game/${roomId}/state`);
    }

    async getGameStatus(roomId) {
        return this.request(`/game/${roomId}/status`);
    }

    // ===== BOT =====
    async createBotGame(goFirst = true) {
        return this.request('/bot/create', {
            method: 'POST',
    ...
    async makeBotMove(gameId, position, version) {
        return this.request(`/bot/${gameId}/move`, {
            method: 'POST',
            body: JSON.stringify({ position, version }),
        });
    }
    ...
    async getBotGame(gameId) {
        return this.request(`/bot/${gameId}`);
    }

    async getBotGames() {
        return this.request('/bot/user/games');
    }

    // ===== REPLAY =====
    async getReplay(roomId) {
        return this.request(`/replay/${roomId}`);
    }

    async getGameHistory() {
        return this.request('/replay/user/history');
    }
}
